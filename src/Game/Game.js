...
export function GamePage() {
  ...
  const checkCordinates = async (e, name) => {
    ...
    positionY = (top - 70) / img.current.offsetHeight;
              ...
              {scores.map((score, index) => {
                return (
                  <div key={index} className="score">
                    <p>{index + 1}:</p> <p className="name">{score.name}</p>{" "}
                    <p>{score.time}</p>
                  </div>
                );
              })}
      ...
      {!started ? (
        <div className="screen">
          <div className="welcome-message">
            <h2>
              Have you ever played where's waldo? I made something similar with
              a cool picture by Egor Klyuchnyk, you will have to find 3
              character
            </h2>
